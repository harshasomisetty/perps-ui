...
export async function openPosition(
  ...
  let positionAccount = findProgramAddressSync(
    [
      ...
      new PublicKey("GBrsFAWty2G2WfsP8XWmzDTrBj8Dafx8ZXpXHD1Th6Y9").toBuffer(),
      [1],
    ],
  ...
  //   let testPositionAccount = (
  //     await findProgramAddress("position", [
  //       publicKey,
  //       pool.poolAddress,
  //       pool.tokens[getTokenAddress(payToken)]?.custodyAccount,
  //       [1],
  //     ])
  //   ).publicKey;

  //   console.log("test Posioition", testPositionAccount.toString());
  ...
  try {
    ...
    console.log(
      "custody",
      pool.tokens[getTokenAddress(payToken)]?.custodyAccount.toString()
    );
    console.log("pool", pool.poolAddress.toString());
    ...
    let tx = await perpetual_program.methods
      ...
      .accounts({
        ...
        position: new PublicKey("DXT1P6HQxw4HyFymXtpWEEjvhCBytBskFyEEAKq34MSA"),
