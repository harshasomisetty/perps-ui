...
export const TRX_URL = (txid: string) => `https://solscan.io/tx/${txid}?cluster=devnet`;
...
export async function awaitTransactionSignatureConfirmation(
    { connection, txid, timeout, confirmations = 1 }
        :
        { connection: Connection, txid: string, timeout: number, confirmations: Number }
) {
    let done = false;
                ...
                (async () => {
                    try {
                        ...
                        if (!done) {
                            ...
                            // @ts-ignore
                            else if (!(result.confirmations >= confirmations || result.confirmationStatus === 'finalized')) {
                            } else {
...
export const sendTransaction = async ({ connection, transaction, wallet, enableSigning = true }: xx) => {
...
export const sendRawTransaction = async (
    { connection, txid, rawTransaction }
        :
        { connection: Connection, txid: string, rawTransaction: any }
) => {
    const timeout = 60000, confirmLevel = 'processed';
...
export const sendSignedTransactionAndNotify = async (
    { connection, transaction, successMessage, failMessage, signTransaction, enableSigning = true }
        :
        {
            connection: Connection;
            transaction: Transaction;
    ...
    const { txid, rawTransaction } = await sendTransaction({ connection, transaction, wallet: { signTransaction }, enableSigning });
